...
module.exports = class extends yeoman {
	...
	prompting() {
		...
		if (typeof this.options.fuse == 'undefined') {
			isFuse = false;
		} else {
			isFuse = this.options.fuse;
		}
	...
	writing() {
		...
		let skipList = [];
		if (showWsdl2Rest) {
			// if we are handling wsdl2rest, we know what files to skip
			for(var i=0; i<skipListForWsdl2RestFiles.length; i++) {
				skipList.push(skipListForWsdl2RestFiles[i]);
			}

			if (isFuse) {
				skipList.push('pom.xml.wsdl2rest');
			} else {
				skipList.push('pom.xml.wsdl2rest.fuse');
			}
		} else {
			skipList.push('pom.xml.wsdl2rest');
			skipList.push('pom.xml.wsdl2rest.fuse');
			if (isFuse) {
				skipList.push('pom.xml');
			} else {
				skipList.push('pom.xml.fuse');
			}
		}
		...
		for (var i = 0; i < this.files.length; i++) {
			var skipFile = testFileForSkip(skipList, this.files[i]);

			if (!skipFile) {
				if (this.files[i].startsWith('pom.xml') && 
					(this.files[i].endsWith('.wsdl2rest') || this.files[i].endsWith('.fuse'))) {
...
function testFileForSkip(skipList, file) {
	for(var i=0; i<skipList.length; i++) {
		if (skipList[i] == file) return true;
	}
}
